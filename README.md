...
| `final-reviewer` | 서브에이전트 | 병합본 최종 검수 (7항목) + 수정 + 재빌드 |

### final-reviewer 검수 항목 (v3.2 신규)

| # | 검수 항목 | 처리 |
|---|----------|------|
| 1 | 미번역 영문 검출 | 직접 번역 or 제거 |
| 2 | 이미지 캡션 본문 유출 | 중복 영문 제거 |
| 3 | 서식 이상 감지 (전체 이탤릭 등) | 본문 이탤릭 해제 |
| 4 | 아티팩트 잔존 | 즉시 제거 |
| 5 | 헤딩 구조 검증 | 볼드→헤딩 변환 |
| 6 | 용어집 일관성 전수 검사 | 불일치 수정 |
| 7 | 빈 줄 보존 확인 | 경고 |

수정 발생 시 DOCX/EPUB 자동 재빌드.
